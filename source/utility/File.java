...
public class File {
    ...
    public static Vertex[] parseObj(String objFilePath, boolean flipZ) throws Exception {
	    ...
	    final int zFlip = flipZ ? -1 : 1;
        for (String fileLine; (fileLine = reader.readLine()) != null;) {
            String[] lineParts = fileLine.split(" ");

            // Parsing the data
            if (Objects.equals(lineParts[0], "v")) {
                xyzBuffer.add(new Float3(Float.parseFloat(lineParts[1]), Float.parseFloat(lineParts[2]), Float.parseFloat(lineParts[3]) * zFlip));
            }
                ...
                normBuffer.add(new Float3(Float.parseFloat(lineParts[1]), Float.parseFloat(lineParts[2]), Float.parseFloat(lineParts[3]) * zFlip));
            }
